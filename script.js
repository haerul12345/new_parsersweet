...
function applyTheme(isSweet) {
    ...
    if (themeBtn && themeIcon && themeText) {
        themeIcon.textContent = isSweet ? '🍫' : '🧁';
        themeText.textContent = isSweet ? 'Switch to Bitter Mode' : 'Switch to Sweet Mode';
    }
} */
/* const THEME_STORAGE_KEY = "parserSweetTheme";
...
function applyTheme(isSweet) {
    ...
    if (icon && text) {
        icon.textContent = isSweet ? "🍫" : "🧁";
        text.textContent = isSweet ? "Switch to Bitter Mode" : "Switch to Sweet Mode";
    }
}

(function initTheme() {
    const saved = localStorage.getItem(THEME_STORAGE_KEY);
    const isSweet = saved === "sweet";
    applyTheme(isSweet);
    ...
    btn.addEventListener("click", () => {
        const nextSweet = !document.body.classList.contains("sweet-mode");
        applyTheme(nextSweet);
        localStorage.setItem(THEME_STORAGE_KEY, nextSweet ? "sweet" : "bitter");
    });
})(); */
...
/* (() => {
  ...
  function applyFlavor(isSweet) {
    document.body.classList.toggle("sweet-mode", isSweet);

    if (icon) icon.textContent = isSweet ? "🍫" : "🧁";
    if (text) text.textContent = isSweet
...
function initTheme() {
    ...
    const isSweet = savedTheme === 'sweet';
    applyTheme(isSweet);
}
...
if (themeBtn) {
    themeBtn.addEventListener('click', () => {
        const isSweet = !document.body.classList.contains('sweet-mode');
        applyTheme(isSweet);
        localStorage.setItem(THEME_STORAGE_KEY, isSweet ? 'sweet' : 'bitter');
    });
}
...
// Balanced extractor (returns substring starting at startIdx or null)
function _getBalanced(text, startIdx) {
...
function showAlert(message, type = 'info', callback = null) {
        ...
        if (!alertText) {
                console.warn("showAlert missing #alertText", message);
                return;
        }
...
document.addEventListener("DOMContentLoaded", function () {
        ...
        if (appVersionEl) {
                appVersionEl.textContent = `Version ${appVersion} © ${yearText} hji`;
        }
        ...
        if (convertBtn && hexInput && asciiResult) {
...
">
${isMultiline ? `Line ${index + 1}: ` : ''}Invalid hex string.</blockquote>`;
                                        return;
                                }
...
<blockquote style="
    ...
    box-shadow:
        ...
        inset 0 1px 0 rgba(255, 255, 255, .03);
">
${isMultiline ? `Line ${index + 1}: ` : ''}${ascii}</blockquote>`;
                        });
      ...
      `;
        ...
        tabs.forEach(tab => {
                tab.addEventListener("click", () => {
                        ...
                        // Show the selected tab content (if exists)
                        const selectedTab = tab.getAttribute("data-tab");
                        const contentEl = document.getElementById(selectedTab);
                        if (contentEl) {
                                contentEl.classList.add("active");
                                contentEl.style.display = "block";
                        }

                        // Hide any other content elements via inline style (keeps backward compatibility)
                        contents.forEach(c => {
                                if (c.id !== selectedTab) c.style.display = "none";
                        });
                });
        });
...
const pasteMtiBtn = document.getElementById("paste-mti-btn");
if (pasteMtiBtn) {
        pasteMtiBtn.addEventListener("click", async () => {
                try {
                        const text = await navigator.clipboard.readText();
...
// Function to extract JSON substring from a string value that contains "Body:" followed by JSON
function extractBodyJsonFromValue(str) {
        console.log('extractBodyJsonFromValue: called');
        if (!str || typeof str !== 'string') {
                console.log('extractBodyJsonFromValue: empty or not a string');
                return null;
        }
        const idx = str.toLowerCase().indexOf('body:');
        console.log('extractBodyJsonFromValue: body: index =', idx);
        if (idx === -1) return null;

        const afterPos = idx + 5;
        const nextObj = str.indexOf('{', afterPos);
        const nextArr = str.indexOf('[', afterPos);
        let start = -1;
        if (nextObj !== -1 && nextArr !== -1) start = Math.min(nextObj, nextArr);
        else start = Math.max(nextObj, nextArr);
        console.log('extractBodyJsonFromValue: start bracket index =', start);
        if (start === -1) return null;

        const cand = (function _getBalanced(text, startIdx) {
                const open = text[startIdx];
                const close = open === '{' ? '}' : ']';
                let depth = 0, inStr = false, esc = false;
                for (let i = startIdx; i < text.length; i++) {
        ...
        console.log('extractBodyJsonFromValue: extracted candidate length =', cand ? cand.length : 0);
        if (!cand) return null;

        try {
                JSON.parse(cand);
                console.log('extractBodyJsonFromValue: candidate is valid JSON');
                return cand;
        } catch (e) {
                console.log('extractBodyJsonFromValue: candidate JSON.parse failed, trying unescape');
                try {
                        const un = JSON.parse(cand);
                        if (typeof un === 'string') {
                                JSON.parse(un);
                                console.log('extractBodyJsonFromValue: unescaped JSON valid');
                                return un;
                        }
                } catch (e2) {
                        console.log('extractBodyJsonFromValue: final parse failed');
                }
        }
        return null;
}

// Simple HTML escaping to prevent issues with displaying raw strings
...
function formatTableRows(data, indentLevel = 0) {
        ...
        for (const key in data) {
                if (data.hasOwnProperty(key)) {
                        ...
                        if (typeof value === 'object' && value !== null) {
                                ...
                                // special-case: render securityData as an inner table
                                if (key === 'securityData' && value !== null) {
                                        console.log('formatTableRows: rendering securityData for key=', key);
                                        let secTable = `
  ...
  `;
                                        for (const f in value) {
                                                ...
                                                // small safety: escape HTML (add escapeHtml helper if not present)
                                                const escField = escapeHtml ? escapeHtml(f) : f;
                                                const escVal = escapeHtml ? escapeHtml(disp) : disp;
                                                secTable += `<tr><td>${escField}</td><td>${escVal}</td></tr>`;
                                        }
                                        secTable += `</tbody></table>`;
                                        rows += `<tr><td>${indent}${key}</td><td>${secTable}</td></tr>`;
                                        continue;
                                }
                                ...
                                // Special handling for 'custom_field' or 'customField' key - look for embedded JSON in the string value
                                if ((key === 'custom_field' || key === 'customField') && typeof value === 'string') {
                                        console.log('formatTableRows: detected custom_field, value length =', (value || '').length);
                                        const bodyJson = extractBodyJsonFromValue(value);
                                        console.log('formatTableRows: extractBodyJsonFromValue returned', bodyJson ? 'JSON' : 'null');
                                        if (bodyJson) {
                                                try {
                                                        const parsed = JSON.parse(bodyJson);
                                                        console.log('formatTableRows: parsed Body JSON successfully');
                                                        const innerTable = generateTable(parsed);

                                                        const originalEsc = escapeHtml(value);
                                                        const combined = `
        <div>
          <strong>Original:</strong>
          <pre style="white-space:pre-wrap;max-height:140px;overflow:auto;margin:6px 0;padding:6px;border:1px solid #ddd;background:#fafafa">
            ${originalEsc}
          </pre>
          <div style="margin-top:8px">${innerTable}</div>
        </div>`.trim();

                                                        rows += `<tr><td>${indent}${key}</td><td>${combined}</td></tr>`;
                                                        continue;
                                                } catch (err) {
                                                        console.log('formatTableRows: JSON.parse failed for extracted Body:', err);
                                                        // fall back to raw output below
                                                }
                                        } else {
                                                console.log('formatTableRows: no Body JSON found in custom_field value');
                                        }
                                }
...
de55Input.addEventListener('input', function () {
...
function decodeDe55() {
        ...
        let raw = (inputElement ? inputElement.value : "").trim().replace(/\s+/g, '').toUpperCase();
        ...
        if (raw.length === 0) {
                outputElement.innerHTML = "";
                //document.getElementById("copyDe55TableBtn").style.display = "none";
                return;
        }
        ...
        if (/^3[0-9A-F]3[0-9A-F]3[0-9A-F]$/i.test(prefix)) {
                console.log("ASCII numeric length prefix detected");
                

                // Convert hex ASCII to actual text
                const asciiLength =
                        String.fromCharCode(
                                parseInt(prefix.substring(0, 2), 16),
                                parseInt(prefix.substring(2, 4), 16),
                                parseInt(prefix.substring(4, 6), 16)
                        );

                lengthInfo = `<blockquote style="text-align:left; width: 50%; 
                                box-shadow:
                                0 20px 40px rgba(0, 0, 0, .08);">DE55 Data Length = ${asciiLength} Bytes</blockquote>`;
...
<div class="de55-container">
    ...
    <table id="de55Table" class="de55-table">
       <thead>
           <tr>
...
<div style="margin-top:10px; text-align:left;">
    <button id="copyDe55TableBtn" type="button">
        📋 Copy Table
    </button>
</div>
...
`;
        ...
        const btn = document.getElementById("copyDe55TableBtn");

        if (btn) {
                btn.style.display = parsedTags.length > 0
                        ? "inline-block"
                        : "none";
        }
...
/* document.addEventListener("click", async (e) => {
        if (e.target.id !== "copyDe55TableBtn") return;

        const table = document.getElementById("de55Table");

        if (!table) {
                alert("No DE55 table available.");
                return;
        }
        ...
        try {
                ...
                alert("DE55 table copied to clipboard.");
        } catch (err) {
                console.error(err);
...
/* document.addEventListener("click", async (e) => {
        ...
        try {
                ...
                canvas.toBlob(async (blob) => {
                        ...
                        showInfoAlert("DE55 table copied as image.");
                });
        } catch (err) { */
...
document.addEventListener("click", async (e) => {
        ...
        try {
                ...
                // Use whichever notification you prefer
                alert("DE55 table copied as image.");

        } catch (err) {

                console.error(err);
                showAlert("Failed to copy image.", "error");
        }
});
...
function parseTLV(hex) {
      ...
      <span class="tooltip-border-box" style="display:none;position:fixed;z-index:9999;background:#fff;border:1px solid #ccc;padding:8px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,0.15);white-space:nowrap;">${tooltipHtml}</span>
        ...
        <span class="tooltip-border-box" style="display:none;position:fixed;z-index:9999;background:#fff;border:1px solid #ccc;padding:8px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,0.15);white-space:nowrap;color:black;">${tooltipHtml}</span>
                ...
                // 5F24 - Application Expiration Date, 5F25 - Application Effective Date, 9A - Transaction Date (YYMMDD in BCD)
                // NOTE: visible data is NOT changed — we only add parsed info to the tooltip.
                if (["5F24", "5F25", "9A"].includes(tag.toUpperCase()) && value.length === 6) {
                        const raw = value.toUpperCase();

                        const bcdToDec = (hexPair) => {
                                const b = parseInt(hexPair, 16);
                                return ((b >> 4) * 10) + (b & 0x0F);
                        };

                        const yy = bcdToDec(raw.slice(0, 2));
                        const mm = bcdToDec(raw.slice(2, 4));
                        const dd = bcdToDec(raw.slice(4, 6));

                        const year = 2000 + yy;
                        const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
                        const monthName = months[mm - 1] || `Invalid(${mm})`;

                        const valid = mm >= 1 && mm <= 12 && dd >= 1 && dd <= 31;
                        const iso = `${year.toString().padStart(4, '0')}-${String(mm).padStart(2, '0')}-${String(dd).padStart(2, '0')}`;
                        const parsedDisplay = valid ? `${iso} (${monthName})` : `Invalid date components`;

                        const humanName = tag.toUpperCase() === "5F24" ? "Application Expiration Date"
                                : tag.toUpperCase() === "5F25" ? "Application Effective Date"
                                        : "Transaction Date";

                        const tooltipHtml = `<div style="font-family:monospace; font-size:12px; color:black; max-width:360px;">
    <strong>${humanName} (YYMMDD, BCD)</strong><br>
    
    Date: ${monthName} ${dd}, ${year} <br>
...
function rearrangeObject(obj, skipAsciiConversion = false) {
        ...
        if ('MTI' in obj) out['MTI'] = obj['MTI'];

        Object.keys(obj).forEach(k => {
                if (k === 'MTI') return;
                const value = obj[k];
                if (asciiKeys.includes(k) && !skipAsciiConversion && typeof value === 'string' && /^[0-9A-Fa-f]+$/.test(value)) {
                        out[k] = hexToAscii(value);
                } else {
                        out[k] = value;
                }
        });
...
/* function parseField60(value) {
  const raw = String(value || '').trim();
  if (!raw) return raw;
 
  const hex = raw.replace(/\s+/g, '');
  if (!/^([0-9A-Fa-f])+$/.test(hex)) {
    return `<div><strong>Raw:</strong> ${raw}</div>`;
  }
 
  let payload = hex;
  let headerInfo = '';
 
  if (payload.slice(0, 2).toUpperCase() === 'E1') {
    const lenByteHex = payload.slice(2, 4);
    const lenByte = parseInt(lenByteHex, 16);
    ...
    if (lenByte & 0x80) {
      ...
      const lenHex = payload.slice(offset, offset + numBytes * 2);
      offset += numBytes * 2;
      length = parseInt(lenHex, 16);
    }
 
    headerInfo = `<div><strong>Field 60 header:</strong> E1 ${lenByteHex} (${length})</div>`;
    payload = payload.slice(offset);
  }
 
  const nestedTable = parseTLV(payload);
  return `
    <div>${headerInfo}</div>
    <div><strong>Raw:</strong> ${hex}</div>
    <div style="margin-top:8px; overflow:auto;">${nestedTable}</div>
  `;
} */

const FIELD_60_DF_TAGS = {
        DF01: "Pre-authorisation transaction type",
        DF02: "The expiry date of the pre-authorisation",
        DF03: "The Logical Terminal ID",
        DF04: "Transaction ID (TID)",
        DF05: "Mastercard’s Transit Transaction Type Indicator",
        DF06: "Mastercard’s Transportation Mode Indicator",
        DF07: "The Transit Deferred Authorisation Indicator",
        DF08: "The Payment Facilitator ID",
        DF09: "The Sub Merchant ID",
        DF0A: "The Sub Merchant Street Address",
        DF0B: "Payment Facilitator Name",
        DF0C: "Mastercard’s Independent Sales Organisation (ISO) ID",
        DF0D: "Mastercard’s Merchant Country of Origin (MCO)",
        DF0E: "Visa’s Merchant Verification Value (MVV)",
        DF0F: "The Mastercard Assigned ID (MAID)",
        DF10: "The American Express Service Establishment (SE) Number",
        DF11: "The Cardholder Activated Terminal (CAT)/Unattended Cardholder Activated Terminal (UCAT) Indicator",
        DF12: "The Acceptor Geographic Coordinates",
        DF13: "The Service Location Geographic Coordinates",
        DF14: "MasterCard Transaction Link Identifier (TLID)",

        // add more DF tags here later
};

function formatField60Tag(tag, valueHex) {
        const tagName = FIELD_60_DF_TAGS[tag] || tagsList["_" + tag] || "";

        // preserve original casing of the value
        let displayValue = valueHex;
...
function parseField60(value) {
        ...
        const e1Index = hexUpper.indexOf("E1");
        ...
        let details = "";
        let payload = hex.slice(e1Index); // preserve original case here

        if (e1Index > 0) {
                details += `<div><strong>Data Length:</strong> ${hex.slice(0, e1Index)}</div>`;
        }

        const payloadUpper = payload.toUpperCase();
        if (!payloadUpper.startsWith("E1") || payload.length < 4) {
                return `<div>${details}<div><strong>Raw:</strong> ${raw}</div></div>`;
        }

        payload = payload.slice(2);
        let lengthByteHex = payload.slice(0, 2);
        let length = parseInt(lengthByteHex, 16);
        let lengthHeader = lengthByteHex;
        let offset = 2;

        if (length & 0x80) {
                const numLenBytes = length & 0x7F;
                const lenHex = payload.slice(offset, offset + numLenBytes * 2);
                length = parseInt(lenHex, 16);
                lengthHeader += lenHex;
                offset += numLenBytes * 2;
        }

        details += `<div><strong>Field 60 length header:</strong> ${lengthHeader} (${length})</div>`;

        const nestedHex = payload.slice(offset, offset + length * 2); // preserve case
        const nestedTable = parseField60Nested(nestedHex);

        return `
    ${details}
    <div><strong>Raw:</strong> ${raw}</div>
    <div style="margin-top:8px; overflow:auto;">${nestedTable}</div>
  `;
}

function parseField60Nested(nestedHex) {
        let table = '<table style="width:100%; border-collapse:collapse;" border="1">';
        table += "<tr><th>Tag</th><th>Length</th><th>Value</th></tr>";

        let i = 0;
        while (i < nestedHex.length) {
                let tag = nestedHex.substr(i, 2);
                i += 2;
                if ((parseInt(tag, 16) & 0x1F) === 0x1F) {
                        tag += nestedHex.substr(i, 2);
                        i += 2;
                }

                let lengthByteHex = nestedHex.substr(i, 2);
                i += 2;
                let length = parseInt(lengthByteHex, 16);

                if (length & 0x80) {
                        const numLenBytes = length & 0x7F;
                        const lenHex = nestedHex.substr(i, numLenBytes * 2);
                        length = parseInt(lenHex, 16);
                        i += numLenBytes * 2;
                }

                const valueHex = nestedHex.substr(i, length * 2);
                i += length * 2;

                const tagName = FIELD_60_DF_TAGS[tag] || tagsList["_" + tag] || "";
                const valueDisplay = formatField60Tag(tag, valueHex);

                table += `<tr><td>${tag}${tagName ? `<br><small><i>${tagName}</i></small>` : ""}</td><td>${length}</td><td>${valueDisplay}</td></tr>`;
        }
...
/* function parseField60(value) {
  ...
  let details = '';
  let payload = hex.slice(e1Index);
 
  if (e1Index > 0) {
    details += `<div><strong>Data Lenght:</strong> ${hex.slice(0, e1Index)}</div>`;
  }
 
  if (!payload.startsWith('E1') || payload.length < 4) {
    return `<div>${details}<div><strong>Raw:</strong> ${raw}</div></div>`;
  }
 
  payload = payload.slice(2); // remove the E1 tag
  let lengthByteHex = payload.slice(0, 2);
  let length = parseInt(lengthByteHex, 16);
  let lengthHeader = lengthByteHex;
  let offset = 2;
  ...
  const nestedHex = payload.slice(offset, offset + length * 2);
  const tlvTable = parseTLV(nestedHex);
...
function createTableFromObject(obj, isNested = false, isBreakdown = false) {
        ...
        // build ordered key list: MTI first (if present), then numeric keys ascending, then other keys
        const iterateKeys = (() => {
                const keys = Object.keys(obj);
                if (keys.length === 0) return keys;
                const hasMTI = keys.includes('MTI');
                const others = keys.filter(k => k !== 'MTI');

                const numeric = others.filter(k => /^\d+$/.test(k)).sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
                const nonNumeric = others.filter(k => !/^\d+$/.test(k)).sort((a, b) => a.localeCompare(b));

                return hasMTI ? ['MTI', ...numeric, ...nonNumeric] : [...numeric, ...nonNumeric];
        })();
...
`;
                ...
                else if (key === "007" && typeof value === "string" && value.length === 10) {
                        const mm = parseInt(value.slice(0, 2), 10);
                        const dd = parseInt(value.slice(2, 4), 10);
                        ...
                        if (mm >= 1 && mm <= 12 && dd >= 1 && dd <= 31) {
                                formatted = `${months[mm - 1]} ${dd}, ${hh}:${mi}:${ss}`;
                        }

                        table += `<tr><td>${key}</td><td>${value} (${formatted})</td></tr>`;
                }
                else if (key === "012" && typeof value === "string" && value.length === 6) {
                        const hh = value.slice(0, 2);
                        const mi = value.slice(2, 4);
                        const ss = value.slice(4, 6);

                        let formatted = `${hh}:${mi}:${ss}`;
      ...
      <div><strong>Length:</strong> ${length}</div><div><strong>Data:</strong> ${data}</div>`;
                ...
                /*     else if (key === "060" && value !== null) {
                      const field60Table = parseField60(value);
                      table += `<tr><td>${key}</td><td>${field60Table}</td></tr>`;
                    } */

                else if (key === "060" && value !== null) {
                        const field60Html = parseField60(value);
                        table += `<tr><td>${key}</td><td>${field60Html}</td></tr>`;
                }
                        ...
                        // 4-9: Original STAN (6)
                        // 10-14: Original Local Transaction Time (hhmmss) (6)
                        // 14-19: Original Local Transaction Date (MMDD) (4)
                        ...
                        // Old DE90 parsing (commented out)
                        //const origTime = raw.slice(10, 16);
                        //const origDate = raw.slice(16, 20);

                        //new DE90 parsing (best-effort slicing)
                        const origDate = raw.slice(10, 14); // MMDD
                        const origTime = raw.slice(14, 20); // hhmmss
                        ...
                        */
                        ...
                        const formatMMDD = (mmdd) => {
                                ...
                                if (month < 1 || month > 12) return mmdd; // fallback if invalid
                        ...
                        detailsTable += row('Original Local transaction Date (Field 13) (MMDD)', `${origDate || ''} ${dateFormatted ? `(${dateFormatted})` : ''}`);
                        detailsTable += row('Original Local transaction Time (Field 12) (hhmmss)', `${origTime || ''} ${timeFormatted ? `(${timeFormatted})` : ''}`);
...
// --- Extract-and-paste helper: supports raw JSON or mixed text ---
// Place at end of script.js
/* (function () {
  function extractJsonFromText(text) {
    if (!text || typeof text !== 'string') return null;
    // prefer {...} then fallback to [...]
  ...
  function tryPrettyJson(jsonText) {
    try {
  ...
  function triggerTransformOnTextarea(textarea) {
    if (!textarea) return;
    // dispatch events for listeners wired to input/change
    textarea.dispatchEvent(new Event('input', { bubbles: true }));
    textarea.dispatchEvent(new Event('change', { bubbles: true }));
 
    // 1) If parseJson exists in module scope, call it
    try {
    ...
    // 2) Then try an exposed/global parseJson
    try {
      if (window && typeof window.parseJson === 'function') {
        window.parseJson();
        return;
      }
    } catch (e) {  }
 
    // 3) Fallback to several common transform function names if defined
    const candidates = [
      'transformJsonInput',
      'transformJson',
      'parseJsonInput',
  ...
  if (pasteButton) {
    pasteButton.addEventListener('click', async () => {
      try {
        ...
        const candidate = extractJsonFromText(txt) || txt;
        const pretty = tryPrettyJson(candidate) || candidate;
        if (jsonInput) jsonInput.value = pretty;
        triggerTransformOnTextarea(jsonInput);
      } catch (err) {
  ...
  if (jsonInput) {
    jsonInput.addEventListener('paste', (e) => {
      try {
        ...
        const extracted = extractJsonFromText(clipboardText);
        if (extracted) {
          e.preventDefault();
          const pretty = tryPrettyJson(extracted) || extracted;
          jsonInput.value = pretty;
          jsonInput.selectionStart = jsonInput.selectionEnd = jsonInput.value.length;
          triggerTransformOnTextarea(jsonInput);
        }
        // if no extracted JSON found, leave default paste behavior
      } catch (err) {
  ...
  // Ensure parseJson is available globally as well (module + non-module compatibility)
  try {
    if (typeof parseJson === 'function' && typeof window !== 'undefined') {
